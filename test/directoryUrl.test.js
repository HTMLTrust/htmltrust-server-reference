...
test('directory URLs fall back to the request origin', () => {
  ...
  assert.equal(directoryKeyUrl(request, 'key 1', {}), 'http://localhost:3000/api/keys/key%201');
});
...
test('directory URLs use the configured public base URL', () => {
  ...
  assert.equal(directoryKeyUrl(request, 'abc', env), 'https://directory.example/api/keys/abc');
});
